...
void espRX(){
  ...
  int idx = response.indexOf("{\"house\":");
  if(idx > -1){
    response = response.substring(idx, response.length()-9);
    int idx2 = response.indexOf("}1,CLOSED");
    if(idx2 > -1){
      response = response.substring(0,idx2);
    }
    Serial.print("\r\n\r\n");
